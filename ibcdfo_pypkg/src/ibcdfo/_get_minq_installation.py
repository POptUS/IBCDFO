...
def get_minq_installation():
    ...
    :return: ``(SHA, installation)`` where
        * ``SHA`` is the SHA of the MINQ git commit required by this version
          of IBCDFO
    ...
    VALID_MINQ_SHA = COMMIT_INFO[0]
    assert len(VALID_MINQ_SHA) == 40
    ...
    installation = {
        ...
        "is_valid": (git_sha == VALID_MINQ_SHA)
    }

    return VALID_MINQ_SHA, installation
