...
def pounders(fun, X_0, n, nf_max, g_tol, delta_0, m, L, U, Prior=None, Options=None, Model=None):
    ...
    if Prior is None:
        ...
        if not all(np.atleast_2d(Prior["X_init"])[Prior["xk_init"]] == X_0):
            print("jeff", np.atleast_2d(Prior["X_init"])[Prior["xk_init"]])
            print("jeff", X_0)

        assert all(np.atleast_2d(Prior["X_init"])[Prior["xk_init"]] == X_0), "Starting point X_0 doesn't match row in Prior['X_init']"
