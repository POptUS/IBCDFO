...
def run_user_friendly(Ffun, X_0, n, nf_max, g_tol, delta_0, m, Low, Upp, hfun=None, concurrent=False):
    r"""
    ...
    if hfun is None:
        hfun = {"hfun": h_leastsquares, "combinemodels": combine_leastsquares}
    if set(hfun) != {"hfun", "combinemodels"}:
        raise ValueError("Invalid hfun configuration")
    hfun["spsolver"] = create_trsp_solver(TRSP_SOLVER_MINQ5)

    # ----- OPTIMIZE!
    if concurrent:
        return pounders_concurrent(Ffun, X_0, n, nf_max, g_tol, delta_0, m, Low, Upp, Options=hfun)

    return pounders(Ffun, X_0, n, nf_max, g_tol, delta_0, m, Low, Upp, Options=hfun)
