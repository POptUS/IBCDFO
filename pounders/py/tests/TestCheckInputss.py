...
class TestLotsOfFeatures(unittest.TestCase):
    ...
    def test_checkinputts5(self):
        gtol_to_fail = 0
        flag = checkinputss(self.fun, self.X0, self.n, self.npmax, self.nfmax,
                gtol_to_fail, self.delta, self.nfs, self.m, self.F0,
                self.xkin, self.L, self.U)[0]
        assert flag == -1

    def test_checkinputts6(self):
        delta_to_fail = 0
        flag = checkinputss(self.fun, self.X0, self.n, self.npmax, self.nfmax,
                self.gtol, delta_to_fail, self.nfs, self.m, self.F0,
                self.xkin, self.L, self.U)[0]
        assert flag == -1
    ...
    def test_checkinputts7(self):
        ...
        flag = checkinputss(self.fun, self.X0, self.n, self.npmax, self.nfmax,
                self.gtol, self.delta, self.nfs, self.m, F0_to_fail,
                self.xkin, self.L, self.U)[0]
        assert flag == -1
    ...
    def test_checkinputts8(self):
        ...
        flag = checkinputss(self.fun, self.X0, self.n, self.npmax, self.nfmax,
                self.gtol, self.delta, self.nfs, self.m, F0_to_warn,
                self.xkin, self.L, self.U)[0]
        assert flag == 0
    ...
    def test_checkinputts10(self):
        xkin_to_fail = -1
        flag = checkinputss(self.fun, self.X0, self.n, self.npmax, self.nfmax,
                self.gtol, self.delta, self.nfs, self.m, self.F0,
                xkin_to_fail, self.L, self.U)[0]
        assert flag == -1

    def test_checkinputts11(self):
        L_to_fail = np.hstack((self.L, self.L))
        flag = checkinputss(self.fun, self.X0, self.n, self.npmax, self.nfmax,
                self.gtol, self.delta, self.nfs, self.m, self.F0, self.xkin,
                L_to_fail, self.U)[0]
        assert flag == -1
