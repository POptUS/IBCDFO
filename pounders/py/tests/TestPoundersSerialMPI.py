...
class TestPoundersMPI(unittest.TestCase):
    def test_benchmark_pounders_mpi(self):
        ...
        os.makedirs(out_serial, exist_ok=True)
        COMM.Barrier()
        ...
        for row, (nprob, n, m, factor_power) in enumerate(dfo):
            ...
            fname_serial = os.path.join(out_serial, f"pounders4py_serial_{base}.mat")

            # ---------------- Skip if already computed ----------------
            if os.path.exists(fname_serial):
            ...
            # --- Serial run ---
            X, F, hF, flag, xk_best = ibcdfo.run_pounders(
                Ffun_batch,
                X_0,
                n,
                nf_max,
            ...
            sp.io.savemat(fname_serial, Results_serial)
