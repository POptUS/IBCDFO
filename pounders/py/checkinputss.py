...
def checkinputss(Ffun, X_0, n, np_max, nf_max, g_tol, delta_0, nfs, m, X_init, F_init, xk_in, Low, Upp):
    ...
    if np_max < n + 1 or np_max > int(0.5 * (n + 1) * (n + 2)):
        ...
        raise ValueError("Error: np_max not in [n+1, 0.5 * (n+1) * (n+2)]")
    ...
    if not isinstance(Low, np.ndarray):
        raise TypeError("Error: Low must be Numpy array")
    if Low.ndim != 1:
        raise ValueError("Error: Low must be 1D Numpy array")
    if len(Low) != n:
    ...
    if not isinstance(Upp, np.ndarray):
        raise TypeError("Error: Upp must be Numpy array")
    if Upp.ndim != 1:
        raise ValueError("Error: Upp must be 1D Numpy array")
    if len(Upp) != n:
    ...
    if X_init.shape[0] != nfs:
        raise ValueError("Error: number of initial points nfs does not match input X_init")
    elif X_init.shape[1] != n:
        raise ValueError("Error: X_init does not contain the right number of coordinates")
    if nfs > 0:
    ...
    if F_init.shape[0] != nfs:
        raise ValueError("Error: number of starting f values nfs does not match input F_init")
    elif F_init.shape[1] != m:
        raise ValueError("Error: F_init does not contain the right number of residuals")
    if nfs > 0:
