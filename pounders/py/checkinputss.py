...
def checkinputss(Ffun, X0, n, npmax, nfmax, g_tol, delta, nfs, m, F_init, xkin, L, U):
    """
    checkinputss(Ffun,X0,n,npmax,nfmax,g_tol,delta,nfs,m,F_init,xkin,L,U) -> [flag,X0,npmax,F_init,L,U]
    ...
    flag  [int] = 1 if inputs pass the test
                = 0 if a warning was produced (X0,npmax,F_init,L,U are changed)
    ...
    if not callable(Ffun):
        ...
        flag = -1
        return [flag, X0, npmax, F_init, L, U, xkin]
