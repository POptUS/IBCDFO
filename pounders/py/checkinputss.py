...
def checkinputss(Ffun, X_0, n, np_max, nf_max, g_tol, delta, nfs, m, X_init, F_init, xkin, Low, Upp):
    """
    checkinputss(Ffun,X_0,n,np_max,nf_max,g_tol,delta,nfs,m,F_init,xkin,Low,Upp) -> [flag,X_0,np_max,F_init,Low,Upp]
    ...
    if not callable(Ffun):
        ...
        flag = -1
        return [flag, X_0, np_max, F_init, Low, Upp, xkin]
    ...
    if np.min(Upp - Low) <= 0:
        ...
        flag = -1
        return [flag, X_0, np_max, F_init, Low, Upp, xkin]
    if np.min([np.min(X_0[xkin, :] - Low), np.min(Upp - X_0[xkin, :])]) < 0:
