...
while np < np_max || np_max == n + 1
    if Nd(i) <= Pars(2) && ~ismember(i, Mind)
        ...
        try
            ...
            if valid_i
                rowvec_len = 1 + size(D, 2);   % length of [1 D(i,:)]
            else
                rowvec_len = NaN;
            end

            % k should be a valid insertion position for qrinsert (1 .. mR+1)
            valid_k = isfinite(k) && (k == round(k)) && k >= 1 && k <= (mR + 1);

            % Compose a single assert-style error with all the facts
            diag = sprintf([ ...
                '\nQRINSERT failure (caught at formquad) with MATLAB id: %s\n' ...
                'Message: %s\n' ...
                'i=%g (integer=%d, in[1,%d]=%d)\n' ...
                'np=%g  -> k=np+1=%g (integer=%d, in[1,%d]=%d)\n' ...
                'size(Q)=%s, size(R)=%s, size(D)=%s\n' ...
                'size(R) rows=mR=%d, cols=nR=%d\n' ...
                'rowvec length (expected 1+size(D,2))=%s\n' ...
                'Notes:\n' ...
                '  - i must index a row of D (1..size(D,1)).\n' ...
                '  - k=np+1 must satisfy 1..mR+1.\n' ...
                '  - length([1 D(i,:)]) should equal size(R,2)=%d.\n'], ...
                ME.identifier, ME.message, ...
                i, i == round(i), size(D, 1), valid_i, ...
                np, k, k == round(k), mR + 1, valid_k, ...
                mat2str(szQ), mat2str(szR), mat2str(szD), ...
                mR, nR, ...
                mat2str(rowvec_len), ...
                nR);

            error('POUNDERS:qrinsert:AssertionFail', '%s', diag);
        end
        Ly = Ny * Qy(:, n + 2:np + 1);
