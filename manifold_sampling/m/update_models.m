function [valid, Gres, Hres, X, F, h, nf, Hash] = update_models(hfun, Ffun, n, p, nf, nfmax, xkin, delta, F, X, h, Hres, fq_pars, geometry_pt_flag, Hash, tol, LB, UB)
    Cres = F(xkin, :);
    ...
    if np < n && geometry_pt_flag % Must obtain and evaluate bounded geometry points
        [Mdir, np] = bmpts(X(xkin, :), Mdir(1:n - np, :), LB, UB, delta, fq_pars.Par(3));
        for i = 1:min(n - np, nfmax - nf)
            [nf, X, F, h, Hash] = call_user_scripts(nf, X, F, h, Hash, Ffun, hfun, X(xkin, :) + Mdir(i, :), tol, LB, UB);
            D = Mdir(i, :);
            for j = 1:p
