...
% [X, F, h, xkin, flag] = manifold_sampling_primal(hfun, objective, X0, L, U, nf_max, 'linprog');
...
[X, F, h, xkin, flag] = manifold_sampling_primal(hfun, X0, X0, L, U, nf_max, 'linprog');
assert(flag == -1, "Should have failed");

objective = @(x) x;
[X, F, h, xkin, flag] = manifold_sampling_primal(hfun, objective, X0, [L 1], U, nf_max, 'linprog');
assert(flag == -1, "Should have failed");
