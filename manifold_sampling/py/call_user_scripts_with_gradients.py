...
def call_user_scripts(nf, X, F, Grad, h, Hash, Ffun, hfun, x_in, tol, L, U, allow_recalc=0):
    ...
    nf += 1
    X[nf] = xnew
    F[nf], Grad[nf] = Ffun(X[nf])
    h[nf], _, hashes_at_nf = hfun(F[nf])
    Hash[nf] = hashes_at_nf
    if np.any(np.isnan(F[nf, :])) or np.any(np.isnan(Grad[nf, :, :])):
    ...
    return nf, X, F, Grad, h, Hash, hashes_at_nf
