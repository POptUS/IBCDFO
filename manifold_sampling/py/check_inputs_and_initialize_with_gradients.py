...
def check_inputs_and_initialize(x0, F0, Grad0, nfmax):
    x0 = x0.squeeze()
    n = int(len(x0))
    p = int(len(F0))
    delta = 0.1
    ...
    Grad = np.concatenate((np.expand_dims(Grad0, 0), np.zeros((nfmax - 1, p, n))), axis=0)
    h = np.zeros((nfmax, 1))
    Hash = {}
    ...
    return n, delta, printf, fq_pars, tol, X, F, Grad, h, Hash, nf, trust_rho, xkin, Hres
